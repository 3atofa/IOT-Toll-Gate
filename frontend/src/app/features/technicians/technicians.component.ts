...
export class TechniciansComponent implements OnInit {
  ...
  load(): void {
    ...
    this.techApi.getAll({
      ...
      error: () => { this.feedback.errorToast('Failed to load technicians'); this.loading = false; },
    });
  }
  ...
  save(): void {
    if (!this.formData.fullName) { this.feedback.errorToast('Full name is required'); return; }
    if (!this.formData.monthlySalary && this.formData.monthlySalary !== 0) {
      this.feedback.errorToast('Monthly salary is required'); return;
    }
    this.saving = true;
    ...
    obs.subscribe({
      next: () => {
        ...
        this.saving = false;
        this.feedback.successToast(this.editingId ? 'Technician updated' : 'Technician added');
        this.load();
      },
      error: (err) => {
        this.saving = false;
        this.feedback.errorToast(err?.error?.error || 'Failed to save');
      },
    });
  }
  ...
  delete(t: Technician): void {
    ...
    this.techApi.delete(t.id).subscribe({
      next: () => { this.feedback.successToast('Technician deleted'); this.load(); },
      error: () => this.feedback.errorToast('Failed to delete'),
    });
  }
  ...
  specClass(s: TechnicianSpecialization): string {
    const map: Record<TechnicianSpecialization, string> = {
      hardware: 'bg-orange-100 text-orange-700', software: 'bg-blue-100 text-blue-700',
      network: 'bg-cyan-100 text-cyan-700', electrical: 'bg-yellow-100 text-yellow-700',
